...
productHelpers.getAllProducts().then((products)=>{
  ...
  res.render('admin/view-products',{admin:true,products,admin,category});
})
});
...
router.post('/edit-product/:id', upload.array('image', 4),(req,res)=>{
  ...
  productHelpers.updateProduct(req.params.id,{name:caName,category:category,price:price,description:description,photos:photos}).then(()=>{
...
router.get('/add-category',verifyLogin,(req,res)=>{
  res.render('admin/add-category',{admin:true,user:false,guest:false,"categoryError":req.session.categoryError})
...
router.get('/dashboard',verifyLogin,async(req,res)=>{
  let totalPriceSevenDay=await adminHelpers.getAmountLastSevenDay()
   ...
   let totalProductQuantitySevenDay=await adminHelpers.getTotalProductQuantityLastSevenDays()
   let totallProductQuantityAMonth=await adminHelpers.getTotalProductQuantityLastMonth()
   let totallProductQuantityAyear=await adminHelpers.getTotalProductQuantityLastYear()
...
router.post('/download',async(req,res)=>{
  ...
  let totalCustomer=await userHelpers.totalUser()
  const data = [
    { days: 'A week', sales: totalPriceSevenDay, orders: totalOrderSevenDay, products: totalProductQuantitySevenDay, customers: userInLastSevenDay },
    { days: 'A month', sales: totalPriceAMonth, orders: totalOrderAMonth, products: totallProductQuantityAMonth, customers: userInLastMonth },
    { days: 'A year', sales: totalPriceAyear, orders: totalOrderAyear, products: totallProductQuantityAyear, customers: userInLastYear }
  ];
