...
const transporter = nodemailer.createTransport({
  ...
  secure:true,
  auth: {
    user: "abhidhsh2001@gmail.com",
    pass: "zoivzmcjmtzerdru",
  },
});
...
router.post("/otp", async (req, res) => {
  ...
  if (otp == req.body.otp) {
    const { name, email, password, cpassword } = req.session.userDetails;
...
router.post("/fetch-products", async (req, res) => {
  try {
    ...
    const Catproducts = await productHelpers.getCatProductByPage(
      Cname,
      skip,
      pageSize
    );
